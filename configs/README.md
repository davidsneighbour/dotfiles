...
| `fonts/` | Font families (JetBrains Mono, Recursive, Monaspace Argon, system/symbol fonts) installed to `~/.fonts`. See [`fonts/READ_ABOUT_LICENSES.md`](./fonts/READ_ABOUT_LICENSES.md) for per-family licensing and [`fonts/dotfiles/DESIGN-typography.md`](./fonts/dotfiles/DESIGN-typography.md) for the dotfiles typography set. | Dotbot (`~/.fonts`). |
