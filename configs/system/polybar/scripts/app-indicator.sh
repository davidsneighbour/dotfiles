...
if echo "${title}" | grep -qE '\([0-9]+\)'; then
  echo "%{F#FF9580}%{A1:~/.config/polybar/scripts/toggle-window.sh --wmclass \"${WMCLASS}\":}${ICON}%{A}%{F-}"
  exit 0
fi

# detect hidden state
if xprop -id "${win}" WM_STATE | grep -q Iconic; then
  echo "%{F#F8F8F2}%{A1:~/.config/polybar/scripts/toggle-window.sh --wmclass \"${WMCLASS}\":}${ICON}%{A}%{F-}"
else
  echo "%{F#414D58}%{A1:~/.config/polybar/scripts/toggle-window.sh --wmclass \"${WMCLASS}\":}${ICON}%{A}%{F-}"
fi
