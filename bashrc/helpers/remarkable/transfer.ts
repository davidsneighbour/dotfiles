...
function parseBooleanEnv(
  ...
  throw new CliError(
    `Invalid boolean environment value: "${value}". Use 1, 0, true, or false.`,
  );
}
...
function parseArguments(argv: string[]): ParsedArguments {
  ...
  if (id !== undefined) {
    parsedArguments.id = id;
  }

  if (name !== undefined) {
    parsedArguments.name = name;
  }
...
function createConfig(argv: string[]): Config | null {
  ...
  const restartXochitlDefault = parseBooleanEnv(
    process.env['RESTART_XOCHITL_DEFAULT'],
    false,
  );
  ...
  const config: Config = {
    ...
    remarkableXochitlDir:
      process.env['REMARKABLE_XOCHITL_DIR'] ??
      '.local/share/remarkable/xochitl/',
    restartXochitl: parsed.restartToggle
...
function validateConfig(config: Config): void {
  ...
  if (config.direction === 'down') {
    ...
    if (filters.length === 0) {
      throw new CliError(
        'For --down, specify --all, --id <uuid>, or --name <text>.',
      );
    }
...
function getSupportedExtension(filePath: string): SupportedExtension {
  ...
  throw new CliError(
    `Unknown extension: ${extension || '(none)'}, skipping ${filePath}`,
  );
}
 ...
 */
async function prepareUploadDocument(
  sourceFile: string,
  ...
  await writeFile(
    join(workDirectory, `${uuid}.metadata`),
    createMetadata(sourceFile, extension),
    'utf8',
  );
  await writeFile(
    join(workDirectory, `${uuid}.content`),
    createContent(extension),
    'utf8',
  );
...
async function uploadDocument(
  ...
  try {
    ...
    await runCommand(
      'scp',
      ['-r', `${workDirectory}/.`, targetDirectory],
      config.verbose,
    );
  } catch (error: unknown) {
...
async function uploadToRemarkable(config: Config): Promise<void> {
  ...
  try {
    ...
    if (config.restartXochitl) {
      ...
      await runCommand(
        'ssh',
        [config.remarkableHost, 'systemctl restart xochitl'],
        config.verbose,
      );
      console.log('Done.');
    }
...
function parseRemoteMetadata(
  ...
  try {
    ...
    if (error instanceof Error) {
      throw new CliError(
        `Could not parse metadata for ${uuid}: ${error.message}`,
      );
    }
  ...
  const deleted =
    typeof parsed['deleted'] === 'boolean' ? parsed['deleted'] : false;
  const type = typeof parsed['type'] === 'string' ? parsed['type'] : '';
  const visibleName =
    typeof parsed['visibleName'] === 'string' &&
    parsed['visibleName'].trim() !== ''
      ? parsed['visibleName']
      : uuid;
...
async function remoteFileExists(
  ...
  try {
    await captureCommand(
      'ssh',
      [config.remarkableHost, 'test', '-f', remotePath],
      config.verbose,
    );
    return true;
  } catch (error: unknown) {
    if (config.verbose) {
      ...
      console.log(
        `Remote file missing or inaccessible: ${remotePath}. ${message}`,
      );
    }
 ...
 */
async function listDownloadableRemoteDocuments(
  config: DownConfig,
 ...
 */
function filterRemoteDocuments(
  ...
  if (config.name !== undefined) {
    ...
    return documents.filter((document) =>
      document.visibleName.toLowerCase().includes(needle),
    );
  }
...
async function downloadFromRemarkable(config: DownConfig): Promise<void> {
  ...
  const selectedDocuments = sortRemoteDocuments(
    filterRemoteDocuments(availableDocuments, config),
  );
  ...
  for (const document of selectedDocuments) {
    ...
    console.log(
      `Downloading ${document.visibleName} (${document.uuid}) to ${outputPath}`,
    );
...
async function rawBackupFromRemarkable(config: RawBackupConfig): Promise<void> {
  ...
  if (config.safe) {
    ...
    await runCommand(
      'ssh',
      [config.remarkableHost, 'systemctl stop xochitl'],
      config.verbose,
    );
  }
  ...
  try {
    ...
    if (config.safe) {
      ...
      await runCommand(
        'ssh',
        [config.remarkableHost, 'systemctl start xochitl'],
        config.verbose,
      );
    }
  }
...
async function run(config: Config): Promise<void> {
  ...
  if (config.direction === 'down') {
    ...
    if (config.id !== undefined) {
      downConfig.id = config.id;
    }

    if (config.name !== undefined) {
      downConfig.name = config.name;
    }

    await downloadFromRemarkable(downConfig);
