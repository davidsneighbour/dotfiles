 ...
 * @returns {string | null}
 */
function normalizeTagEntry(entry: unknown): string | null {
 ...
 */
async function fetchRaindropTags(config: Config): Promise<string[]> {
  ...
  try {
    ...
    throw new Error(
      `Raindrop.io API returned invalid JSON: ${
  ...
  if (!isRecord(body)) {
    throw new Error('Raindrop.io API returned a non-object JSON response.');
  }
  ...
  if (tags.length === 0) {
    throw new Error(
      `No tags found in API response. Response keys: ${
