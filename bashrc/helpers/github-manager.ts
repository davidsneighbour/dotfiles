...
const commandRegistry: Record<CommandName, CommandDefinition> = {
    ...
    pull: {
        name: "pull",
        description: "Pull updates for each local repository using its upstream/default remote.",
        run: async ({ config }) => {
            const repos = inventoryLocalRepositories(config.basePath, config.verbose);
    ...
    "remote-list": {
        name: "remote-list",
        description: "List remote GitHub repositories for an owner and fetch metadata plus tags.",
        run: async ({ config }) => {
            const records = await fetchRemoteRepositoryInventory(config);

            for (const record of records) {
                const topics = (record.repo.repositoryTopics ?? []).map((topic) => topic.name).join(", ");
                ...
                console.log(`  default-branch: ${record.repo.defaultBranchRef?.name ?? ""}`);
                console.log(
    ...
    "clone-by-topic": {
        ...
        run: async ({ config }) => {
            ...
            const filtered = filterRepositoriesByTopics(repositories, config.topicFilters);

            if (filtered.length === 0) {
                log("warn", `No repositories matched topics: ${config.topicFilters.join(", ")}`);
                return;
            }
    ...
    "sync-all": {
        name: "sync-all",
        description: "Run pull, status, audit, and optionally clone-by-topic in one pass.",
        run: async ({ config }) => {
            const localRepos = inventoryLocalRepositories(config.basePath, config.verbose);
...
function parseArgs(argv: string[]): CliConfig {
    ...
    if (!commandToken || !isCommandName(commandToken)) {
        throw new Error(`Unknown command "${commandToken ?? ""}". Run with --help.`);
    }
...
function inventoryLocalRepositories(basePath: string, verbose: boolean): LocalRepository[] {
...
function getDefaultRemoteName(repoPath: string, upstreamRef: string | null): string | null {
...
async function pullRepository(repo: LocalRepository, config: CliConfig): Promise<void> {
    ...
    if (!branch) {
        log("warn", `Skipping ${repo.name}: current branch could not be determined.`);
        return;
    }
...
function fetchRepository(repoPath: string, verbose: boolean, dryRun: boolean): void {
    if (dryRun) {
        if (verbose) {
...
function getConflictFiles(repoPath: string): string[] {
    const result = execCommand("git", ["diff", "--name-only", "--diff-filter=U"], {
        cwd: repoPath,
        allowFailure: true,
        silent: true,
    });
...
function getAheadBehind(repoPath: string): { ahead: number; behind: number } {
    const result = execCommand("git", ["rev-list", "--left-right", "--count", "HEAD...@{upstream}"], {
        cwd: repoPath,
        allowFailure: true,
        silent: true,
    });
...
function getRemoteOnlyCommits(repoPath: string, allowedAuthorEmails: string[]): RemoteOnlyCommit[] {
    const result = execCommand("git", ["log", "HEAD..@{upstream}", "--format=%H%x09%an%x09%ae"], {
        cwd: repoPath,
        allowFailure: true,
        silent: true,
    });
    ...
    const allowedEmails = new Set(allowedAuthorEmails.map((email) => email.trim().toLowerCase()));
...
function auditRepository(repo: LocalRepository, config: CliConfig): RepositoryAuditResult {
    try {
        ...
        return {
            ...
            stateOk,
            remoteOnlyCommits,
            hasRemoteCommitsByOthers: remoteOnlyCommits.some((commit) => !commit.isOurs),
            broken: false,
            brokenReason: null,
        };
    } catch (error: unknown) {
...
function printAuditResult(result: RepositoryAuditResult, verbose: boolean): void {
    const isInteresting = !result.stateOk || result.hasRemoteCommitsByOthers || result.broken;
    ...
    console.log(`  remote-commits-by-others: ${result.hasRemoteCommitsByOthers ? "yes" : "no"}`);
...
async function fetchRemoteRepositories(config: CliConfig): Promise<GhRepositorySummary[]> {
    const args = [
        "repo",
        "list",
        config.owner,
    ...
    if (!isGhRepositorySummaryArray(parsed)) {
        throw new Error("Unexpected response while parsing GitHub repository list.");
    }
...
async function fetchRemoteRepositoryInventory(
    ...
    for (const repo of repositories) {
        const result = await runRepoOperationAsync(repo.nameWithOwner, "fetch-tags", async () => {
...
function filterRepositoriesByTopics(
    ...
    const requiredTopics = new Set(topicFilters.map((topic) => topic.toLowerCase()));
...
function isGhRepositorySummaryArray(value: unknown): value is GhRepositorySummary[] {
    return Array.isArray(value) && value.every((item) => isGhRepositorySummary(item));
}
...
function isGhRepositoryTag(value: unknown): value is GhRepositoryTag {
    ...
    if (!isString(value.name) || !isString(value.zipball_url) || !isString(value.tarball_url)) {
        return false;
    }
