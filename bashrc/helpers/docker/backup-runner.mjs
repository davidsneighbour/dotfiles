...
function printHelp() {
  const commandName = basename(process.argv[1] ?? 'backup-runner');
  console.log(`Usage: ${commandName} [options]

Options:
  --root <path>               Root directory to scan recursively (default: current working directory)
  --destination <path>        Directory where backup archives are written (default: <root>/backups)
...
function parseArgs(argv) {
  const options = {
    ...
    help: false,
    afterEachCommand: undefined,
    afterAllCommand: undefined,
  };
...
function interpolate(template, variables) {
  return template.replace(/\$\{([a-zA-Z0-9_]+)\}/g, (_match, name) => variables[name] ?? '');
}

async function execCommand(command, cwd, options = {}) {
  const { dryRun = false, verbose = false, env = {} } = options;

  if (dryRun) {
    log('INFO', `[dry-run] ${cwd}: ${command}`, verbose);
    return;
  }
...
async function findTasks(rootDirectory, verbose, options = {}) {
  const results = [];
  const ignoredDirectories = new Set((options.ignoredDirectories ?? []).map((value) => resolve(value)));
...
function toBackupTomlConfig(data) {
  if (data.mode && data.mode !== 'compose-copy' && data.mode !== 'command') {
    throw new Error(`Unsupported mode in backup.toml: ${String(data.mode)}`);
  }

  if (data.compression && data.compression !== 'tar.gz') {
    throw new Error(`Unsupported compression in backup.toml: ${String(data.compression)}`);
  }

  if (data.data_paths && !Array.isArray(data.data_paths)) {
...
async function createTarGzFromPaths(cwd, archivePath, relativePaths, options) {
...
async function createTarGzFromDirectory(sourceDirectory, archivePath, options) {
...
async function executeTomlTask(taskPath, options, hostName) {
...
function detectScriptType(taskPath) {
...
async function executeScriptTask(taskPath, options, hostName) {
  const startedAt = Date.now();
  const taskDirectory = dirname(taskPath);
...
async function runAfterCommand(commandTemplate, result, options, hostName) {
...
async function validateRootDirectory(rootDirectory) {
...
async function main() {
  ...
  const destinationDirectory = resolve(rootDirectory, options.destination);
